...
/// @brief Decode string as Unicode point
/// @param it Pointer to start from
/// @param end Pointer to end of string
/// @return Unicode point code
char32_t decode_utf8(const char *&it, const char *end)
{
  ...
  if (c < 0x80) { // 1 byte
    ...
    res = ((it[0] & 0x1F) << 6) | (it[1] & 0x3F);
    it += 2;
    return res;
  } else if ((c & 0xF0) == 0xE0) { // 3 bytes
    if (it + 2 >= end)
      throw std::runtime_error("Invalid UTF-8");
    res = ((it[0] & 0x0F) << 12) | ((it[1] & 0x3F) << 6) | (it[2] & 0x3F);
    it += 3;
    return res;
  } else if ((c & 0xF8) == 0xF0) { // 4 bytes
    if (it + 3 >= end)
      throw std::runtime_error("Invalid UTF-8");
    res = ((it[0] & 0x07) << 18) | ((it[1] & 0x3F) << 12) | ((it[2] & 0x3F) << 6) | (it[3] & 0x3F);
    it += 4;
    return res;
  }
  throw std::runtime_error("Invalid UTF-8");
...
// cdg buffer
std::vector<char> fl;
nlohmann::json jsonMasks;

// initcdg loads the color table and clears screen. The passed color is the desired color of the
// text
...
// draw block to screen. block is a 2d array, as CDG blocks are 12x6 px. Char blocks are stored in
// values.h
...
// draw string onto cdg(string,row(really uint8_t but char is easier to use),col,blue(if the text is
// colored(the chosen color) or white))
char drawString(std::string str, char row, char col, bool blue)
{
  ...
  while (it < end) {
    auto unicodePoint = decode_utf8(it, end);
    std::string position{ std::to_string(unicodePoint) };

    if (!jsonMasks.contains(position)) {
      std::cerr << "Invalid symbol position in JSON: " << position << std::endl;
    ...
    for (int x = 0; x < 24; x++) {
      for (int a = 0; a < 12; a++) {
        if (x < 12 && a < 6) {
          // b1
          b1[x][a] = jsonMasks[position][x][a];
        } else if (x < 12 && a >= 6) {
          // b2
          b2[x][a - 6] = jsonMasks[position][x][a];
        } else if (x >= 12 && a < 6) {
          // b3
          b3[x - 12][a] = jsonMasks[position][x][a];
        } else if (x >= 12 && a >= 6) {
          // b4
          b4[x - 12][a - 6] = jsonMasks[position][x][a];
        }
      }
    }
...
void convlrc2cdg(const char *flin, const char *flout, int r, int g, int b)
{
  ...
  while (std::getline(infile, line)) {
    // lrc says the first timestamp needs to be formatted like [], and subsequent ones in the
    // line like <>. Just make it simpler
  ...
  initcdg(blue);
  std::vector<com> commandsLine1; // line 1 buffer
  std::vector<com> commandsLine2; // line 2 buffer
  bool draw = false;
  // work out what words will go on what line
  for (const auto &command : commands) {
    if (draw) {
      commandsLine2.push_back(command);
    } else {
      commandsLine1.push_back(command);
    }
    ...
    if (command.newline) {
      if (draw) {
        ...
        for (const auto &cmd : commandsLine1) {
          total += cmd.text;
        }

        for (const auto &cmd : commandsLine2) {
          total2 += cmd.text;
        }
        // draw "white" text
        ...
        for (const auto &cmd : commandsLine1) {
          wait(cmd.time);
          pos_left = drawString(cmd.text, 0x05, pos_left, true);
        }
        pos_left = 0x03;
        for (const auto &cmd : commandsLine2) {
          wait(cmd.time);
          pos_left = drawString(cmd.text, 0x08, pos_left, true);
        }
        commandsLine1.clear();
        commandsLine2.clear();
        clrScr();
